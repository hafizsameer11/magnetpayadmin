...
export const ESCROWS: EscrowContract[] = [
  ...
  },
  {
    ...
    flag: "Auto-release in 2 days — buyer has not confirmed receipt",
  },
  {
  ...
  },
  {
    ...
    flag: "Dispute DSP-44021 opened — customs hold, NCC compliance docs",
  },
  {
  ...
  },
  {
    ...
    flag: "Full refund issued — wrong language variant",
  },
  {
    ...
    flag: "Items damaged on arrival — admin override refund",
  },
  {
    ...
    flag: "Bulk order — buyer requested AQL 2.5 inspection",
  },
  {
    ...
    flag: "Auto-release window expired — buyer never confirmed, no dispute opened",
  },
];
        ...
        <table className="w-full text-[12px]">
          ...
          <tbody>
            {pager.slice.map((e) => {
              ...
              const total = e.milestones.length || 1;
              const pct = e.totalNGN > 0 ? Math.round((e.releasedNGN / e.totalNGN) * 100) : Math.round((released / total) * 100);
              const displayId = e.id.startsWith("ESC-") ? e.id : `ESC-${e.id.slice(0, 5).toUpperCase()}`;
              const done = e.status === "released" || e.status === "refunded";
              return (
                <tr key={e.id} className="border-t hover:bg-black/[0.015] transition" style={{ borderColor: T.border }}>
                  <td className="px-2 py-3 pl-4">
                    <Link to="/admin/escrow/$id" params={{ id: e.id }} className="font-bold tabular-nums hover:underline" style={{ color: T.ink, fontFamily: "'JetBrains Mono', monospace" }}>{displayId}</Link>
                  ...
                  <td className="px-2 py-3">
                    {e.orderId && e.orderId !== "—" ? (
                      ...
                      >
                        {e.orderId.startsWith("ORD-") ? e.orderId : `ORD-${e.orderId.slice(0, 6).toUpperCase()}`}
                      </Link>
                    ) : (
                      <span className="font-medium tabular-nums" style={{ color: T.sub, fontFamily: "'JetBrains Mono', monospace" }}>—</span>
                    )}
                  ...
                  <td className="px-2 py-3 text-[11px] tabular-nums" style={{ color: done ? T.muted : e.daysLeft < 0 ? T.danger : e.daysLeft <= 3 ? T.warn : T.sub, fontFamily: "'JetBrains Mono', monospace" }}>
                    {done ? "completed" : e.daysLeft < 0 ? `${Math.abs(e.daysLeft)}d overdue` : `${e.daysLeft}d left`}
                  </td>
                  <TableActionTd>
                    <ActionMenu
                      label={`Actions for escrow ${displayId}`}
                      items={[
                        {
                          id: "view",
                          label: "View contract",
