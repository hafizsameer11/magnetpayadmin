...
export function EscrowQueuePage({
  ...
  return (
    <AdminShell
      ...
      actions={
        <button onClick={() => demo("Export queued — CSV will be ready in ~30s", "success")} className="h-9 px-3 rounded-lg text-[12px] font-semibold flex items-center gap-1.5" style={{ background: T.surface, border: `1px solid ${T.border}`, color: T.ink }}>
    ...
    >
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
        ...
        <KPI label="Avg held" value={rows.length ? fmtNGN(Math.round(held / rows.length)) : "—"} />
