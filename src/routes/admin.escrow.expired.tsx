...
export const Route = createFileRoute("/admin/escrow/expired")({
  head: () => ({ meta: [{ title: "Expired escrow — MagnetPay Admin" }] }),
  component: Page,
});

const EXPIRED_LIKE = new Set(["EXPIRED", "CANCELLED"]);
...
function Page() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const filtered = data.filter((e) => EXPIRED_LIKE.has(e.status.toUpperCase()));
  ...
  return (
    <AdminShell
      title="Expired escrow"
      description="Escrows in EXPIRED or CANCELLED status requiring reconciliation."
      breadcrumbs={[{ label: "Admin", to: "/admin" }, { label: "Escrow", to: "/admin/escrow" }, { label: "Expired" }]}
    >
      <div className="rounded-xl overflow-hidden" style={{ background: T.surface, border: `1px solid ${T.border}` }}>
        ...
        {loading ? (
          ...
          <p className="p-6 text-center text-[12px]" style={{ color: T.muted }}>
            No expired or cancelled escrows.
          </p>
        ) : (
          rows.map((e, i) => (
            <div
