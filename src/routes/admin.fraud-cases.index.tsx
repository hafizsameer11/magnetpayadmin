...
function Page() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const fraud = audit.filter(
          (a) => a.action.toLowerCase().startsWith("fraud") || a.action.toLowerCase().includes("fraud"),
        );
        if (!cancelled) setRows(fraud);
      } catch (e) {
        if (!cancelled) {
  ...
  return (
    <AdminShell
      title="Fraud cases"
      description="Audit events whose action starts with or includes “fraud”. Empty is OK without a dedicated fraud API."
