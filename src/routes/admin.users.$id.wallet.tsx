...
import { fetchAdminWallets } from "@/lib/api";
import { Loader2 } from "lucide-react";

export const Route = createFileRoute("/admin/users/$id/wallet")({
  head: () => ({ meta: [{ title: "Users Id Wallet — MagnetPay Admin" }] }),
  component: Page,
});
...
function Page() {
  ...
  return (
    <AdminShell
      title="Users Id Wallet"
      description="Live data from API. Empty until records exist."
      breadcrumbs={[{ label: "Admin", to: "/admin" }, { label: "Users Id Wallet" }]}
    >
      {loading ? (
