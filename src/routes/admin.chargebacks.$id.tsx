...
import { fetchAdminEscrows } from "@/lib/api";
import { Loader2 } from "lucide-react";

export const Route = createFileRoute("/admin/chargebacks/$id")({
  head: () => ({ meta: [{ title: "Chargebacks Id — MagnetPay Admin" }] }),
  component: Page,
});
...
function Page() {
  ...
  return (
    <AdminShell
      title="Chargebacks Id"
      description="Live data from API. Empty until records exist."
      breadcrumbs={[{ label: "Admin", to: "/admin" }, { label: "Chargebacks Id" }]}
    >
      {loading ? (
