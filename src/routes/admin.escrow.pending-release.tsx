...
export const Route = createFileRoute("/admin/escrow/pending-release")({
  head: () => ({ meta: [{ title: "Pending release — Escrow" }] }),
  component: Page,
});

const PENDING = new Set(["FUNDED", "IN_PROGRESS", "ACTIVE", "AWAITING_FUNDS"]);
...
function Page() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const filtered = data.filter((e) => PENDING.has(e.status.toUpperCase()));
  ...
  return (
    <AdminShell
      ...
      description="Funded or in-progress escrows awaiting release."
      breadcrumbs={[
